...
```
The command names the first three columns year, month and day respectively. It is not clear what the last 24 columns represent.
...
hist(rain.df$daily)
```
The histogram shows that there are negative rainfall values, which does not make sense.
...
```{r}
neg_vals = which(rain.df$daily < 0)
rain.df.fixed = rain.df[-neg_vals,]
...
```{r}
flights %>% filter(month == 10, day == 12)
```
