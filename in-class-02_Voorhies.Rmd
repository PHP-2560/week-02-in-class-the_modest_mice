...
#labels the columns year, month, day, and then the rest of the columns are labeled 0 through 23
````
...
# there are negative rain values -- cannot have negative rain. Need to remove negative values.

removeVar<-which(rain.df$daily<0)
removeVar
rain.df.fixed <- rain.df[-removeVar,]
...
names(flights)
?mean
#average delay (both arrival & departure) for all flights
summarise(flights, avg_delay = mean(arr_delay+dep_delay,na.rm=TRUE))
...
flights %>%
group_by(dest) %>%
summarise(nFlights = n_distinct("flight") %>%
summarise(nPlanes=n_distinct("flight"))%>%
print(nFlights)
nPlanes
